...
const fetchUser = async () =>{
    try{
        const res = await axios.get(`http://localhost:3001/api/Staff/scoreEva/${id_eva}`,{headers:{Authorization:`Bearer ${token}`}})
        user.value = res.data
    }catch(err){
...
const fetchTopic = async () =>{
    try{
        const res = await axios.get(`http://localhost:3001/api/Staff/scoreEva/indicate/${id_eva}`,{headers:{Authorization:`Bearer ${token}`}})
