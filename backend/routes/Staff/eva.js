...
router.get('/',verifyToken,requireRole('ฝ่ายบุคลากร'),async (req,res) => {
    try{
        const [rows] = await db.query(`select * from tb_eva,tb_member,tb_system where tb_eva.id_sys=tb_system.id_sys and tb_eva.id_member=tb_member.id_member and status_eva=1 order by id_eva desc`)
        res.json(rows)
    }catch(err){
        console.error('Error Get',err)
        res.status(500).json({message:'Error Get'})
...
router.get('/all',verifyToken,requireRole('ฝ่ายบุคลากร'),async (req,res) => {
    try{
        const [rows] = await db.query(`select * from tb_eva,tb_member,tb_system where tb_eva.id_sys=tb_system.id_sys and tb_eva.id_member=tb_member.id_member order by id_eva desc`)
        res.json(rows)
    }catch(err){
        console.error('Error Get',err)
        res.status(500).json({message:'Error Get'})
...
router.get('/:id_eva',verifyToken,requireRole('ฝ่ายบุคลากร'),async (req,res) => {
    try{
        ...
        const [rows] = await db.query(`select * from tb_eva where id_eva='${id_eva}' order by id_sys desc`)
