...
router.get('/:id_eva',verifyToken,requireRole('ฝ่ายบุคลากร'),async (req,res) => {
    try{
        ...
        const [after] = await db.query(`select id_commit,first_name,last_name,tb_member.id_member,level_commit as role from tb_commit,tb_eva,tb_member where tb_commit.id_eva='${id_eva}' and tb_eva.id_eva=tb_commit.id_eva and tb_commit.id_member=tb_member.id_member`)
...
router.post('/:id_eva',verifyToken,requireRole('ฝ่ายบุคลากร'),async (req,res) => {
    try{
        ...
        const m = req.body
        const v = m.map( c => [c.id_member,id_eva,c.role,'n'] )
        await db.query(`insert into tb_commit (id_member,id_eva,level_commit,status_commit) values ?`,[v])
