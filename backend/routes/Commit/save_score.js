...
router.post('/:id_eva',verifyToken,async (req,res) => {
    try{
        ...
        const [[commitRow]] = await db.query(
            `select * from tb_commit where id_eva=? and id_member=?`,
            [id_eva,id_member]
        )
        var statusCommit = 0
        ...
        //กรรมการบันทึกคะนนรวม 3 คนให้ update status_eva = 3
        const [sumCommit] = await db.query(
            `select * from tb_commit where id_eva=? and status_commit=?`,
            [id_eva,'y']
        )
        if(sumCommit.length === 3){
            await db.query(
                `update tb_eva set status_eva=? where id_eva=?`,
                [3,id_eva]
            )
        }
